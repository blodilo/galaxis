# Galaxis — Produktionsmechanik v1.0

**Status:** Design finalisiert · Implementation ausstehend (AP4)
**Abhängigkeiten:** `economy_v1.0.md`, `game-params_v1.5.yaml`, Planetengenerierung, Fraktionssystem
**Datum:** 2026-03-22
...
Alle balancierbaren Parameter leben in `game-params_v1.5.yaml` unter `production:`.
...
- Pro Planet kann jeder verfügbare Rohstoff ein eigenes Bergwerk bekommen.
- Output/Tick steigt mit Level; durch Aufzug-Kapazität gedeckelt.

| Level | Output / Tick | Baukosten (Assembler) |
|-------|---------------|-----------------------|
| Lv1   | 10 E / Tick   | Kolonieschiff (Starter) |
| Lv2   | 20 E / Tick   | 30 E Stahl + 5 E Basiskomponente |
| Lv3   | 35 E / Tick   | 60 E Titanstahl + 15 E Basiskomponente |
| Lv4   | 55 E / Tick   | 100 E Titanstahl + 30 E Basiskomponente |
| Lv5   | 80 E / Tick   | 150 E Titanstahl + 50 E Basiskomponente + 10 E NavComp |
...
## 8. Bootstrap — Kolonieschiff-Starterpaket
...
## 9. Vollständiger Kreislauf — Flussdiagramm
...
Good
  ...
  ├── name: string
  ├── tier: 1–4
  ├── sensitivity_class: I | II | III | IV
  └── mass_per_unit_tonnes: float

ProductionRecipe
  ├── id: string
  ├── facility_type: smelter | refinery | precision_fab | ...
  ├── ticks: int
  ├── inputs[]:  { good_id, quantity: float }
  ├── outputs[]: { good_id, quantity: float }
  └── nominal_efficiency: float     # Basis-η (Lv1, kein Forschungsbonus)
...
ProductionFacility
  ...
  ├── efficiency_float_accumulator: float   # akkumulierter Fraktionsrest
  ├── ticks_remaining: int
  └── status: idle | running | paused_input | paused_output

StationStorage
  ├── station_id: uuid
  ├── sensitivity_class: I | II | III | IV
  ├── capacity_units: float
  ├── contents[]: { good_id, quantity: float }
  └── wrong_class_contents[]: { good_id, quantity: float, decay_accumulated: float }
```
...
**MVP-Kompatibilität:**
Das aktuelle `system_storage` (ein gemeinsamer Pool pro System) ist der **degenerierte Spezialfall**:
alle Knoten implizit vollverbunden, Pipelines ohne Kapazitätslimit.
Die Pipeline-Schicht ist ein Post-MVP-Overlay — keine Datenbankänderungen nötig, nur neues Routing-Layer.

**Nächste Schritte (Post-MVP):**
→ Pipeline-Datenmodell entwerfen (Quellen, Senken, Kanten, Kapazitäten, Güter-Aufteilung)
→ Schiffs-Routing-Integration (AP5)
→ React Flow GUI (AP6 oder eigenes AP8)
...
*Erstellt: 2026-03-22 · Alle Designentscheidungen fixiert · Pipeline-Graph-Konzept ergänzt (Post-MVP)*
